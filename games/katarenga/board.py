...
def draw_board(screen, fonts, board, draw_pieces=True):
    ...
    for y in range(BOARD_SIZE):
        for x in range(BOARD_SIZE):
            # Ensure tile type is a valid string
...
def configure_board(screen, fonts):
    ...
    # Generate initial random quadrants
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                # Confirm and return board
                elif event.key == pygame.K_RETURN:
                    # Validate board before returning
...
# Optional utility functions for board manipulation
...
# Export the key functions for use in other modules
__all__ = [
    'create_random_board',
    'configure_board',
    'draw_board',
