...
HOVER_GREEN = (0, 200, 0)
RED = (255, 0, 0)
BOARD_SIZE = 10  # Changé de 8 à 10 pour inclure les coins
INNER_BOARD_SIZE = 8  # Taille de la zone de jeu principale
TILE_SIZE = 55
TILE_KEYS = ['A', 'B', 'C', 'D']  
...
def validate_board(board):
    ...
    """
    # Vérifier que les coins sont présents
    corner_positions = [(0, 0), (0, 9), (9, 0), (9, 9)]
    for y, x in corner_positions:
        if board[y][x] != 'CORNER':
            return False
    
    # Extraire la zone de jeu 8x8 (sans les bordures et coins)
    inner_board = []
    for y in range(1, 9):
        row = []
        for x in range(1, 9):
    ...
    # Valider les quadrants de la zone de jeu
...
def generate_board_from_quadrants(quadrants):
    ...
    # Placer les quadrants dans la zone de jeu 8x8 (positions 1-8)
    for i in range(2):
        for j in range(2):
...
def draw_board(screen, fonts, board, draw_pieces=True):
    ...
    # Dessiner chaque case du plateau 10x10
    for y in range(BOARD_SIZE):
        for x in range(BOARD_SIZE):
            ...
            if tile_type == 'CORNER':
                ...
                # Rotation des bordures selon leur position
                if y == 0 or y == 9:  # Bordures horizontales
                    tile_image = pygame.transform.scale(CONTOUR_IMAGE, (TILE_SIZE, TILE_SIZE))
                else:  # Bordures verticales
...
def configure_board(screen, fonts):
    ...
    while running:
        ...
        # Générer et dessiner le plateau complet
        ...
        # Dessiner les bordures des quadrants pour montrer les zones cliquables
        for i in range(2):
            for j in range(2):
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                # Vérifier les clics sur les quadrants
                for i in range(2):
                    for j in range(2):
                        idx = i * 2 + j
                        quad_x = board_x + TILE_SIZE + j * quadrant_size
                ...
                # Vérifier les clics sur les coins
                corner_positions = [(0, 0), (0, 9), (9, 0), (9, 9)]
...
def edit_board(screen, fonts, board):
    ...
    y_cursor, x_cursor = 1, 1  # Commencer dans la zone de jeu (pas sur les bordures)
    ...
    while running:
        ...
        # Curseur rouge (seulement dans la zone de jeu)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                # Clic sur les cases de jeu (zone 8x8 centrale)
                for y in range(1, 9):
                    for x in range(1, 9):
                        tile_rect = pygame.Rect(board_x + x * TILE_SIZE, board_y + y * TILE_SIZE, TILE_SIZE, TILE_SIZE)
...
def get_corner_owner(board, corner_pos):
    ...
    if (y, x) == (0, 0) or (y, x) == (0, 9):
        return 'player1'  # Coins du haut pour le joueur 1
    elif (y, x) == (9, 0) or (y, x) == (9, 9):
        return 'player2'  # Coins du bas pour le joueur 2
    return 'neutral'
